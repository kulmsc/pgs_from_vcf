...
#x <- grep("PGS", unlist(strsplit(list.files("refine_other_ss/"), ".", fixed=T)), value=T)
#y <- grep("PGS", unlist(strsplit(list.files("raw_other_ss/"), ".", fixed=T)), value=T)
#todo <- paste0(y[!(y %in% x)], ".txt.gz")

###
for(filename in c("PGS000809.txt.gz")){
#for(filename in list.files("raw_other_ss/")){
#for(filename in todo){
  print(filename)

  #read the summary statistic in
  ss <- read.table(paste0("raw_catalog_sets/", filename), stringsAsFactors=F, header=T, sep='\t')
  print(dim(ss))
  ...
  }#else if("reference_allele" %in% colnames(ss)){
