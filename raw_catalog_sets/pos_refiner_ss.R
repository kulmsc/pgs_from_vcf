...
impute <- read.table("~/athena/doc_score/raw_ss/common_files/impute_rsids", header = F, stringsAsFactors = F)
...
#Now iterating through each of the score summary statistics
x <- unique(grep("PGS", unlist(strsplit(list.files("../pgs_sets/"), ".", fixed=T)), value=T))
y <- grep("PGS", unlist(strsplit(list.files(), ".", fixed=T)), value=T)
todo <- paste0(y[!(y %in% x)], ".txt.gz")
...
for(filename in todo){
  ...
  #get the reference genome of the score file to see if we can match the positions only
  system(paste0("zcat ", filename, " | fgrep build | cut -f2 -d'=' > temp"))
  build_name <- read.table("temp", stringsAsFactors=F)
  system("rm temp")

  if(build_name[,1] == "hg19" | build_name[,1] == "GRCh37"){
    print("pass: reference genome")

    #read the summary statistic in
    ss <- read.table(paste0(filename), stringsAsFactors=F, header=T, sep='\t')
    print(dim(ss))

    if("effect_allele" %in% colnames(ss)){
      print("pass: effect allele")

      if(!("reference_allele" %in% colnames(ss))){
        print("fail: reference allele - going to work around")
        ss <- ss[!is.na(ss$rsID),]
        ...
        comp_allele <- rep("X", nrow(ss))
        comp_allele[ss$effect_allele == "A"] <- "T"
        comp_allele[ss$effect_allele == "T"] <- "A"
        comp_allele[ss$effect_allele == "C"] <- "G"
        comp_allele[ss$effect_allele == "G"] <- "C"

        ref_allele <- rep("X", nrow(ss))
        ref_allele[ss$effect_allele == sub_impute$A1] <- sub_impute$A2[ss$effect_allele == sub_impute$A1]
        ref_allele[ss$effect_allele == sub_impute$A2] <- sub_impute$A1[ss$effect_allele == sub_impute$A2]
        ss$effect_allele[ref_allele == "X"] <- comp_allele[ref_allele == "X"]
        ref_allele[ss$effect_allele == sub_impute$A1 & ref_allele == "X"] <- sub_impute$A2[ss$effect_allele == sub_impute$A1 & ref_allele == "X"]
        ref_allele[ss$effect_allele == sub_impute$A2 & ref_allele == "X"] <- sub_impute$A1[ss$effect_allele == sub_impute$A2 & ref_allele == "X"]

        ss$reference_allele <- ref_allele
        sub_impute <- sub_impute[ss$reference_allele != "X",]
        ss <- ss[ss$reference_allele != "X",]
      }
      ...
      if("rsID" %in% colnames(ss)){
        ...
        print("fail: rsid not present - going to workaround")
      ...
      for(ii in 1:22){
        if(ii %in% out_ss[,1]){
        write.table(out_ss[out_ss[,1] == ii,], paste0("../pgs_sets/", new_name, ".", ii, ".ss"), col.names = T, row.names = F, sep = '\t', quote=F)
        }
      }
     print("done\n")
