...
for(filename in todo){
  ...
  colnames(ss)[colnames(ss) == "other_allele"] <- "reference_allele"
  ...
  if("effect_allele" %in% colnames(ss) & "reference_allele" %in% colnames(ss)){
    print("pass: allele cols present")
    ...
    if("rsID" %in% colnames(ss)){
      ...
      #iterating to make one file for each chromosome
      print("writing out")
      for(ii in 1:22){
        if(ii %in% out_ss[,1]){
