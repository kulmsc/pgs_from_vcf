...
x <- grep("PGS", unlist(strsplit(list.files("pgs_sets/"), ".", fixed=T)), value=T)
y <- grep("PGS", unlist(strsplit(list.files("raw_catalog_sets/"), ".", fixed=T)), value=T)
todo <- paste0(y[!(y %in% x)], ".txt.gz")

###
#for(filename in c("PGS000833.txt.gz", "PGS000148.txt.gz")){
